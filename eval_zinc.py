...
def evaluate_split_performance(
    ...
    df = pd.read_csv('zinc_5ht4.csv')
    df['ProSmith_pred'] = y_pred_0
...
def main():
    ...
    loader_v = load_data(val_csv, embed_path, binary_task, device, gpu)
    roc_plot_path = roc_plot_path.replace(".png", "_val.png")
    evaluate_split_performance(model, loader_v, gpu, device, binary_task, roc_plot_path, output_csv=args.output_csv)
    return
