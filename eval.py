...
def load_model(pretrained_model, device, binary_task):
    ...
    if os.path.exists(pretrained_model):
        ...
        print("Model path is invalid, cannot load pretrained MM_TN model")
    return model


def load_data(val_dir, embed_path, binary_task, device, gpu, batch_size=24, world_size=1):
    ...
    valsampler = DistributedSampler(val_dataset, shuffle = False, num_replicas = world_size, rank = gpu, drop_last = True)
    valloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=1, sampler=valsampler)
    return valloader
...
def main():
    ...
    if version == "T1":
        ...
        # T2
        val_dir = "./data_dir/prosmith_test2/train_val/val.csv"
